...
commander
  .command("add <dependency> <source>").description("Link <source> to update node_modules path for <dependency>")
...
commander.command("clear").description("Remove all livelinks").action(() => {
  setLiveLinks({});
  runLink(null);
  console.log("Cleared all cached links");
});
commander.command("start").description("start running all links defined in 'livelink list'").action(() => {
  console.log("Starting");
  runLink(getLiveLinks());
});
commander.command("addlocal").description("Add all local dependencies and devDependencies (e.g. path starts with a .) as livelinks").action(() => {
...
commander.command("list").description("List livelinks that will be launched at start (all in package.json)".action(() => {
  console.log(JSON.stringify(getLiveLinks(), null, 2));
});
commander.command("remove <dependency>").description("Remove one livelink").action(([dependency]) => {
